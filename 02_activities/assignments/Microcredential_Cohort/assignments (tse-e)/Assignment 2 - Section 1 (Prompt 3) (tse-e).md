...
```
Type 1 slow changing dimensions means that customer addresses will not be retained if a customer moves (i.e. there is no address history). Everytime the customer moves, the column 'customer_address' in the customer table will change to their new address. 

Type 2 slow changing dimensions means that customer addresses will be retained if a customer moves (i.e. there is address history). To retain this history, we could set up another table called 'customer_address_history' with a primary key of customer_address_id, and other columns of customer_id, customer_address, etc. This 'customer' table would then have a one-to-many relationship with the 'customer_address_history' on 'customer_id'. 
```
