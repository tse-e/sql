...
SELECT c.customer_id,c.customer_last_name, c.customer_first_name, 
       SUM(ROUND((cp.quantity * cp.cost_to_customer_per_qty),2)) AS total_spending
...
HINT: you might need to search for strftime modifers sqlite on the web to know what the modifers for month 
and year are! 
Limit to 25 rows of output. */
--QUERY 11
...
SELECT customer_id,
	   STRFTIME('%Y', market_date) AS year, 
	   STRFTIME('%m', market_date) AS month, 
	   SUM(ROUND((quantity * cost_to_customer_per_qty),2)) AS total_spending
FROM customer_purchases
