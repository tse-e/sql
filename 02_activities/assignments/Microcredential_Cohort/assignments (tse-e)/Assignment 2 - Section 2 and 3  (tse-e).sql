...
DROP TABLE IF EXISTS temp.visit_number_descending;

CREATE TABLE temp.visit_number_descending AS
SELECT *,
DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date DESC) as visit
FROM customer_purchases;

SELECT * 
FROM visit_number_descending
WHERE visit = 1;
...
SELECT *, 
COUNT() OVER (PARTITION BY customer_id, product_id ORDER BY transaction_time) AS number_of_times_purchased
FROM customer_purchases
WHERE market_date < '2022-04-29';
...
SELECT *, 
	CASE WHEN product_name LIKE '%-%'
			THEN LTRIM(SUBSTR(product_name,INSTR(product_name, '-')+2)) 
		 ElSE NULL
	END AS description 
FROM product;
...
CREATE TABLE temp.sales_by_dates AS
...
SELECT 'Best Day' AS best_or_worse,market_date, total_sales
FROM ranked_sales
WHERE best_day = 1

UNION

SELECT 'Worst Day' AS best_or_worse,market_date, total_sales
FROM ranked_sales
WHERE worst_day =1;
...
SELECT vendor_name, product_name, SUM(original_price * 5) AS total_amount
